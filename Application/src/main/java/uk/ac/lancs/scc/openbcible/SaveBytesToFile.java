...
public class SaveBytesToFile extends AsyncTask<byte[], Void, Void> {
	...
	protected Void doInBackground(byte[]... arduinoData) {
		...
		//NOTE THESE ARE ONE VALUE OFF BECAUSE THESE ARE ONLY 31 BIT PACKETS BEING SENT
		//SEE: We are reserving the first byte to use as a packet check-sum in our protocol. So the available bytes-per-packet, as far as the uC is concerned, is 31.
		//FROM: http://docs.openbci.com/Hardware/03-Cyton_Data_Format
		...
		//THIS CODE TELLS US that ABOVE, PACKET, 1, 4, MEANS PACKETS 1, 2, AND 3 NOT INCLUDING 4
